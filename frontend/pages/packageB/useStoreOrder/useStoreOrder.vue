<template>
	<view class="orderMain">
		<!-- 自定义导航 -->
		<uniNav title="待使用订单"></uniNav>

		<LoadMoreList :page="page" :data="page.list" noMoreText="没有待核销待订单">
			<view class="orderMain">
				<view class="order -warp30">
					<view class="mb16 orderWarp -bgf" v-for="item in page.list" :key="item.orderId">
						...
						<view class="f f_align mt12" @click="doOrderDetail(item)">
							<u--image :src="item.goodsInfo.goodsBannerUrlList[0]" radius="8" width="80" height="80" />
							<view class="f f_column f_align w100 ml12">
								<view class="f f_align_start f_align">
									<view class="overfont-2">{{item.goodsName}}</view>
								</view>
								<view class="mt8 fz12 tsize overfont">地址：{{item.buyStore.address}}</view>
								<view class="f f_align_center mt8">
									<text class="price mr12">π{{(item.userFinalPrice).toFixed(2)}}</text>
									<text
										class="fz12 textLineThrough tsize">门店价：{{(item.listingPrice).toFixed(2)}}</text>
								</view>
							</view>
						</view>
					</view>
