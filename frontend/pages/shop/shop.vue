<template>
	<view class="container">
		<!-- 自定义导航 -->
		<uniNav type="search" leftIcon="" bgColor="#FFF" class="dropMenu" :placeholder="$t('pages.shop.input')"
			@search="storeMainListKeyWord" @clear="storeMainListKeyWord">
			<template #search_right>
				<view class="f_column f_c_c r_he_order"
					...
					<text class="mt4">{{areaName ? areaName : $t('pages.shop.icon')}}</text>
				</view>
			</template>
		</uniNav>
		...
		<view class="swiper_warp_main">
			<!--  -->
			<view class="middle_ad mb12" v-if="carBannerImg.length && carBannerImg.length > 0">
				<u-swiper imgMode="scaleToFill" :list="carBannerImg" @click="bannerJumps($event)" bgColor="none"
					keyName="adImgUrl" circular height="300rpx"></u-swiper>
			</view>
			<!--  -->
						...
						<view class="f_c_a mt4">
							<view class="f f_column f_align">
								...
								<view class="fz12 price_warp f f_align_center">
									...
									<text
										class="ml8 textLineThrough">{{$t('pages.shop.price')}} {{(item.tmerStoreGoodsInfoEntity.listingPrice).toFixed(2)}}</text>
							...
							<view @click="buyNowGoods(item,item.tmerStoreGoodsInfoEntity)" class="buyNow">{{$t('pages.shop.buy')}}</view>
						</view>
					</view>
				</view>
			</LoadMoreList>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 车店banner查询
			getAd() {
				_getAd('10016003').then(res => {
					if (res && res.code == 0) {
						this.carBannerImg = res.data
					}
				})
			},
			// 查询门店列表信息
			...
			// banner跳转
			bannerJumps(e) {
				this.$toRuntimeUrlIndex(this.carBannerImg[e])
			},
			// 
